...
async def gotify_msg(message):
    token = os.environ.get("GOTIFY_TOKEN")
    base_url = os.environ.get("GOTIFY_SERVER_URL")
    if not token or not base_url:
        print("[error] No GOTIFY_SERVER_URL or GOTIFY_TOKEN, skip the notice")
        return
    async with aiohttp.ClientSession() as session:
...
def gotify_sync(message):
    asyncio.run(gotify_msg(message))
